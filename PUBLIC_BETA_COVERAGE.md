...
| H-05 | 证据矩阵与内容校准层 | Covered for skill beta | `references/evidence-matrix.md` covers 30 high-frequency topics with age band, scene, evidence level, source category, reviewed_at, limits, and escalation thresholds; `scripts/source_freshness.py` checks review freshness. | Future work: deeper per-scenario traceability in long-form notes and production content review tooling. |
...
| H-09 | 埋点、质量评测与回归测试 | Covered for skill beta gate | `references/evaluation-set.jsonl` provides executable regression data; `.github/workflows/public-beta.yml` runs CI; `scripts/release_guardrails.py`, `scripts/beta_kpi_gate.py`, `scripts/run_regression.py`, `scripts/semantic_score.py`, and `scripts/quality_dashboard.py` cover local gate, machine-readable reports, and static dashboard; `references/quality-monitoring.md` defines beta review events and feedback taxonomy. | Production events, BI dashboard, and online replay sample process still require a platform telemetry stack and consent flow. |
...
| --- | --- | --- |
| KPI / events / dashboard | Covered for skill beta gate through `scripts/beta_kpi_gate.py`, `scripts/quality_dashboard.py`, and `references/quality-monitoring.md`. | Production events, dashboards, and retention/helpfulness metrics require a target platform. |
...
| Legal and medical source freshness | Covered for skill beta gate via `references/evidence-matrix.md`, `references/regional-resources.json`, `scripts/source_freshness.py`, and `OPS.md`. | Official materials should still be rechecked before public release or product implementation. |
