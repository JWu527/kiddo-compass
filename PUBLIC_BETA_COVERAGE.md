...
| --- | --- | --- | --- | --- |
| H-01 | 去种子化并清空真实家庭数据 | Covered for skill beta | `.gitignore` / `.clawhubignore` exclude runtime files; `.example.md` templates use privacy-friendly fields; `PUBLISHING.md` includes privacy checks. | Multi-user isolation and account-scoped record creation depend on a future platform layer. |
...
| H-04 | 分离事实、推断、干预与结果的状态模型 | Deferred | Current templates are simpler local Markdown records. | No structured facts / hypotheses / interventions / outcomes schema, confidence, source tags, timestamps, or history model. |
| H-05 | 证据矩阵与内容校准层 | Partial | `references/evidence-matrix.md` covers sleep, feeding, toileting, aggression, and separation with evidence labels. | Needs Top 30 scene coverage, per-advice traceability, and systematic cleanup of unsupported claims across older notes. |
...
| H-09 | 埋点、质量评测与回归测试 | Partial | `references/evaluation-set.md` provides a bilingual manual regression set; `PUBLISHING.md` requires sampling it. | No event schema, BI dashboard, weekly quality report, online replay sample process, or automated prompt regression runner. |
...
| M-02 | 可访问性与低认知负荷版本 | Partial | `SKILL.md` and `routing-guide.md` define super-short / standard / deep output modes. | No UI accessibility checklist, TTS, large-touch controls, undo flows, or usability test record. |
...
| --- | --- | --- |
| KPI / events / dashboard | Partial via manual evaluation set only. | No production events, dashboards, weekly report, or retention/helpfulness metrics. |
...
| Public package safety | Covered for skill beta. | Continue running privacy scans and confirming runtime files are ignored before publishing. |

## Next Coverage Milestones

1. Expand `evidence-matrix.md` to at least 15 topics, then 30.
2. Add per-scenario evidence labels and escalation thresholds to all scenario cards.
3. Replace static learning plan with goal-based tracks.
4. Draft a platform-state design before implementing any App / 小程序 / account-backed version.
5. Automate the manual evaluation set when a test harness is available.
