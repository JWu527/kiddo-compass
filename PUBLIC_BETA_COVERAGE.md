...
| H-05 | 证据矩阵与内容校准层 | Covered for skill beta gate | `references/evidence-matrix.md` covers 30 high-frequency topics with age band, scene, evidence level, source category, reviewed_at, limits, and escalation thresholds; the 20 public-beta scenario cards now carry explicit `Evidence:` labels; `scripts/source_freshness.py` checks review freshness and `scripts/beta_kpi_gate.py` blocks unlabeled scenario cards. | Future work: deeper per-scenario traceability in long-form notes and production content review tooling. |
